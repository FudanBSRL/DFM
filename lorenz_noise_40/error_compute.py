...
def plot_oriRC_fltRC_cmp():
  ...
  ax1.plot(fltRC_T[start:], fltRC_Y[0, start:], c=color[2], linestyle='-.')
  ax1.set_xlim((10,25))
  ax1.set_xticks(xticks)

  ax2.plot(td_T[start:], td_Y[1, start:], c=color[0], linestyle='-')
  ax2.plot(oriRC_T[start:], oriRC_Y[1, start:], c=color[1], linestyle='--')
  ax2.plot(fltRC_T[start:], fltRC_Y[1, start:], c=color[2], linestyle='-.')
  ax2.set_xlim((10,25))
  ax2.set_xticks(xticks)

  ax3.plot(td_T[start:], td_Y[2, start:], c=color[0], linestyle='-')
  ax3.plot(oriRC_T[start:], oriRC_Y[2, start:], c=color[1], linestyle='--')
  ax3.plot(fltRC_T[start:], fltRC_Y[2, start:], c=color[2], linestyle='-.')
  ax3.set_xlim((10,25))
  ax3.set_xticks(xticks)

  fig2.savefig(f'./error_pic/oriRC_fltRC_cmp.svg')
  fig2.savefig(f'./error_pic/oriRC_fltRC_cmp.png')


def plot_oriRC_ad_cmp(ad_ex):
  current_train = 25
  ...
  noise_t, noise_data = read_noise_data('./datas/lorenz_noise_snr40.csv')
  td_T = noise_t
  td_Y = noise_data
...
def plot_fltRC_ad_cmp(ad_ex):
  ...
  fig2.savefig(f'./error_pic/fltRC_ad_cmp.svg')
  fig2.savefig(f'./error_pic/fltRC_ad_cmp.png')


def plot_oriRC_ad_abs_cmp(ad_ex):
  current_train = 25
  target_data = lorenz_ode(time_span=(0, current_train), ic=[-8., 7., 27.], t_eval=torch.linspace(0, current_train, int(40 * current_train + 1)), method='RK45')
  td_T = target_data.t
  td_Y = target_data.y
...
def plot_oriRC_ad_de_cmp(ad_ex):
...
def plot_fltRC_ad_abs_cmp(ad_ex):
  current_train = 25
  target_data = lorenz_ode(time_span=(0, current_train), ic=[-8., 7., 27.], t_eval=torch.linspace(0, current_train, int(40 * current_train + 1)), method='RK45')
  td_T = target_data.t
  td_Y = target_data.y
...
def plot_fltRC_ad_de_cmp(ad_ex):
  target_data = lorenz_ode(time_span=(0, 25), ic=[-8., 7., 27.], t_eval=torch.linspace(0, 25, 1001), method='RK45')
  td_T = target_data.t
  td_Y = target_data.y
  td_dY = (td_Y[:, 2:] - td_Y[:, :-2]) / (td_T[2:] - td_T[:-2]).reshape(1, -1)
...
def plot_real_ode_phase():
  ...
  ax.plot(id_Y[:, 0], id_Y[:, 1], id_Y[:, 2], c=color[2], linewidth=1)
...
def plot_ori_ad_phase():
  noise_t, td_Y = read_noise_data('./datas/lorenz_noise_snr40.csv')
  td_Y = td_Y.T

  ad_T = torch.load('./datas/T_50.pt').detach().numpy()
  ad_Y = torch.load('./datas/ad_Y_50.pt').detach().numpy()
...
def plot_nonoise_noise_phase():
  noise_t, noise_Y = read_noise_data('./datas/lorenz_noise_snr40.csv')
  ...
  ax = fig.add_subplot(111, projection='3d')
  ax.plot(nonoise_Y[start:, 0], nonoise_Y[start:, 1], nonoise_Y[start:, 2], c=color[0], linewidth=1)
  ax.plot(noise_Y[start:, 0], noise_Y[start:, 1], noise_Y[start:, 2], c=color[2], linewidth=1)
...
def plot_ori_flt_phase():
  ...
  trained_net = torch.load(f'./model/model_rb_pre_without_model_ex10.pth')
  flt_Y = trained_net(V(torch.from_numpy(noise_t)).double().reshape(-1, 1)).detach().numpy()
  np.savetxt(f"./csvs/lorenz_noise_snr40_fit.csv", flt_Y.T, delimiter=",")
...
def plot_ad_cmp():
  td_T, td_Y = read_noise_data('./datas/lorenz_noise_snr40.csv')

  ad_T = torch.load('./datas/T_50.pt').detach().numpy()
  ad_Y = torch.load('./datas/ad_Y_50.pt').detach().numpy().T
...
if __name__=='__main__':
  ...
  # plot_oriRC_ad_cmp(ad_ex=50)
  # # # 绘制滤波后RC图像和调整后图像对比
  # plot_fltRC_ad_cmp(ad_ex=ad_data_ex)
  ...
  # plot_oriRC_ad_abs_cmp(ad_ex=ad_data_ex)
  # # # 绘制原始数据RC图像和调整后图像微分方程偏差对比
  # plot_oriRC_ad_de_cmp(ad_ex=ad_data_ex)

  # # # 绘制降噪数据RC图像和调整后图像绝对值偏差对比
  # plot_fltRC_ad_abs_cmp(ad_ex=ad_data_ex)
  # # # 绘制降噪数据RC图像和调整后图像微分方程偏差对比
  # plot_fltRC_ad_de_cmp(ad_ex=ad_data_ex)
  # plot_real_ode_phase()
