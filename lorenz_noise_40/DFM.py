...
def ad_train(pd_net_path, net, num_d, num_epoch, input, label, ksi, Lambda):
    ...
    for epoch in range(num_epoch):
        ...
        if (epoch + 1) % 100 == 0:
            # print(f'epoch {epoch}, mseu {mseu}')
            print(f'epoch {epoch + 1}, loss {loss:f}, mse0 {mse0:f}, mseu {mseu:f}, msed {msed:f}')
            # print(f'epoch {epoch:f}, loss {loss:f}, mseu {mseu:f}, msep {msep:f}, msep_t {msep_t:f}')
            # torch.save(net, pd_net_path)
...
def adjust_data(pd_net_path, num_d, input, label, ksi, Lambda):
    ...
    net = ad_train(pd_net_path=pd_net_path, net=ad_net, num_d=num_d, num_epoch=20000, input=input, label=label, ksi=ksi, Lambda=Lambda)
...
if __name__ == '__main__':
    ...
    # acc_t = data.t
    # acc_data = data.y
    acc_t, acc_data = read_noise_data(f'./datas/lorenz_noise_snr40.csv')
