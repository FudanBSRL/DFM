...
if __name__ == '__main__':
  ...
  id_data = lorenz_ode_id(time_span=(0, 15), ic=ic, t_eval=torch.linspace(0, 15, int(100 * 15 + 1)), method='RK45')
  id_data.t = id_data.t + 10

  color = ['purple', 'red']
  fig1, (ax1, ax2, ax3) = plt.subplots(nrows=3, ncols=1, sharex=False)
  ax1.plot(target_data.t, target_data.y[0, :], c=color[0], linestyle='-', linewidth=0.5)  # 25s
  ...
  ax2.plot(target_data.t, target_data.y[1, :], c=color[0], linestyle='-', linewidth=0.5)  # 25s
  ax2.plot(id_data.t, id_data.y[1, :], c=color[1], linestyle='--') # trian data

  ax2.set_xlabel('t')
  ax2.set_ylabel('y')

  ax3.plot(target_data.t, target_data.y[2, :], c=color[0], linestyle='-', linewidth=0.5)  # 25s
