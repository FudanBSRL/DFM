...
if __name__ == '__main__':
  ...
  id_data = lorenz_ode_id(time_span=(0, 15), ic=ic, t_eval=np.linspace(0, 15, int(40 * 15 + 1)), method='RK45')
  ...
  td_T = torch.from_numpy(target_data.t).double()
  td_Y = torch.from_numpy(target_data.y).double()
  
  # id_T = torch.from_numpy(id_data.t).double()
  id_T = td_T
  id_Y = torch.from_numpy(id_data.y).double()

  id_Y = torch.cat((td_Y[:, 0:401], id_Y[:, 1:]), axis=1)
