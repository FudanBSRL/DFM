...
def plot(acc_data_t, acc_data, predict_t, predict_data):
    ...
    fig1.align_labels()
    fig1.savefig(f'img1/pinn_pic{ex}.png', dpi=500)
...
if __name__ == '__main__':
    ...
    for i in range(int(testtime / time_step)):
        ...
        Y_k = torch.from_numpy(pre_Y[-1:]).reshape(-1, 4).double()
        ...
        train_T = torch.linspace(p_T[0, 0], p_T[-1, 0], round(time_step * 200)).reshape(-1, 1).double()

        T0 = T_k - (p_T_ori[0, 0] + p_T_ori[-1, 0]) / 2

        net = Net(width=20, deep=10, in_num=1, out_num=4).double()
